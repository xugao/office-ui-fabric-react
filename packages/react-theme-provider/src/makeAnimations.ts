...
export function makeAnimations<TStyleSet extends { [key: string]: IStyle }>(
  ...
  return (options: UseStylesOptions = {}) => {
    ...
    if (!value) {
      ...
      const registeredKeyframes = renderKeyframes(renderer, rendererOptions, styles);
...
function renderKeyframes<TStyleSet extends { [key: string]: IStyle }>(
  renderer: StyleRenderer,
  ...
  for (const key in styles) {
    if (styles.hasOwnProperty(key) && keyRegExp.test(key)) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      keyframes[key.split(' ')[1]] = renderer.renderKeyframes(styles[key] as any, rendererOptions);
      delete styles[key];
    }
  }
